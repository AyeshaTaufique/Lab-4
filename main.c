...
struct Contacts
{
char name[100]; char email[100]; char p_num[12];
};


int main(){
struct Contacts *cont = malloc(1*sizeof(struct Contacts)); // an array of structs
int i = 0, choice;
char del_name[100]; int del_num;
int j; while(1){
printf("\n");
...
printf("Enter the email of the contact: "); scanf("%s", cont[i].email);
...
printf("\n");
for(int j = 0; j<i; j++){
printf("%d. %s	%s	%s\n", j+1, cont[j].name, cont[j].email, cont[j].p_num);
}
scanf("%d", &del_num); printf("\n");
...
case 3:
for(int j = 0; j<i; j++){
printf("%s	%s	%s\n", cont[j].name, cont[j].email, cont[j].p_num);
}
break;
...
struct Node *new = createNode(data); new->next = current->next;
current->next = new;
}
else
{
...
int main()
{
struct Node *head1 = createNode(1); insertAfter(head1, 2, 1);
insertAfter(head1, 3, 2); printList(head1);

struct Node *head2 = createNode(2); insertAfter(head2, 3, 2);
insertAfter(head2, 4, 3); printList(head2);

struct Node *temp1 = head1; struct Node *temp2 = head2; struct Node *head3 = NULL; int val;
while (temp1 != NULL && temp2 != NULL)

{
if (temp1->data < temp2->data)
{
val = temp1->data; temp1 = temp1->next;
}
else
{
val = temp2->data; temp2 = temp2->next;
}
...
int nodeCount(struct Node* head){ struct Node* current = head; int count = 0;
...
insertAfter(head, 3, 2); printList(head);
int arr[50], i = 0, count = nodeCount(head); struct Node *temp = head; while(temp!=NULL){
arr[i] = temp->data; temp = temp->next; i++;
}
...
insertAfter(head, 5, 4); printList(head);

int count = nodeCount(head), i = 0; struct Node *temp = head;
struct Node *temp2; while(temp!=NULL){
temp2 = temp->next; if(temp2!=NULL){
temp->next = temp2->next;
}
free(temp2); temp = temp->next; i+=2;
}
